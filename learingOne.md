...
来源网址： http://es6.ruanyifeng.com/#docs/array
作者： john-sky
数组的扩展
...
const contains = (() =>
  Array.prototype.includes
    ...
    : (arr, value) => arr.some(el => el === value))();contains(["foo", "bar"], "baz"); // => false
...
var a1 = [1, 2, 3, 4];var a2 = [for (i of a1) i * 2];
...
var a1 = ['x1', 'y1'];var a2 = ['x2', 'y2'];var a3 = ['x3', 'y3'];
...
来源网址： http://es6.ruanyifeng.com/#docs/object
作者： john-sky
对象的扩展
...
// 报错
var foo = 'bar';var bar = 'abc';var baz = { [foo] };
// 正确
...
var target = { a: { b: 'c', d: 'e' } }var source = { a: { b: 'hello' } }
...
let obj = { one: 1, two: 2 };for (let [k, v] of Object.entries(obj)) {
...
let a = {a: 'a'};let b = {b: 'b'};let c = {c: 'c'};let d = mix(c).with(a, b);
上面代码中，对象a和b被混入了对象c。
...
来源网址： http://es6.ruanyifeng.com/#docs/symbol
作者： john-sky
Symbol
...
var sym = Symbol();Boolean(sym) // true
!sym  // false
if (sym) {
  // ...
}
...
log.levels = {
  DEBUG: Symbol('debug'),
  INFO: Symbol('info'),
  WARN: Symbol('warn'),};log(log.levels.DEBUG, 'debug message');log(log.levels.INFO, 'info message');
下面是另外一个例子。
...
a1[0] = 3;
a1[1] = 4;let a2 = new A2();
a2[0] = 5;
a2[1] = 6;[1, 2].concat(a1).concat(a2)
// [1, 2, 3, 4, [5, 6]]
...
来源网址： http://es6.ruanyifeng.com/#docs/proxy
作者： john-sky
Proxy和Reflect
...
var twice = {
  ...
  return left + right;};var proxy = new Proxy(sum, twice);proxy(1, 2) // 6
...
var handler = {
  ...
  }};var target = { _prop: 'foo', prop: 'foo' };var proxy = new Proxy(target, handler);'_prop' in proxy // false
...
var target = { _prop: 'foo' }var proxy = new Proxy(target, handler)
delete proxy._prop
...
var handler = {
  ...
  }}var target = { prop: 'foo', _bar: 'baz', _prop: 'foo' }var proxy = new Proxy(target, handler)for (let key in proxy) {
  console.log(key);
  // "prop"
}
...
var handler = {
  ...
  }}var target = { _foo: 'bar', baz: 'tar' };var proxy = new Proxy(target, handler);
...
var proxy = new Proxy(target, handler);for (let key of Object.keys(proxy)) {
  console.log(key)}
// "baz"
preventExtensions()
...
var handler = {
  ...
  }}var proto = {};var target = function () {};var proxy = new Proxy(target, handler);
...
来源网址： http://es6.ruanyifeng.com/#docs/function
作者： john-sky
函数的扩展
...
var numbers = [4, 38];add(...numbers) // 42
...
function Timer () {
  this.seconds = 0
  setInterval(() => this.seconds++, 1000)}var timer = new Timer()setTimeout(() => console.log(timer.seconds), 3100)
// 3
...
上面代码之中，只有一个this，就是函数foo的this，所以t1、t2、t3都输出同样的结果。因为所有的内层函数都是箭头函数，都没有自己的this，所以它们的this其实都是最外层foo函数的this。
...
const hasOwnProperty = Object.prototype.hasOwnProperty;function hasOwn(obj, key) {
...
来源网址： http://es6.ruanyifeng.com/#docs/arraybuffer
作者： john-sky
二进制数组
...
var buf = new ArrayBuffer(32);var dataView = new DataView(buf);
...
x1[0] = 1;var x2 = new Uint8Array(buffer);
x2[0]  = 2;
...
var buffer = new ArrayBuffer(8);var i16 = new Int16Array(buffer, 1);
...
var x = new Int8Array([1, 1]);var y = new Int8Array(x);
x[0] // 1
y[0] // 1
...
var x = new Int8Array([1, 1]);var y = new Int8Array(x.buffer);
x[0] // 1
y[0] // 1
...
let ui8 = Uint8Array.of(0, 1, 2);for (let byte of ui8) {
  console.log(byte);}
// 0
// 1
// 2
...
var buffer = new ArrayBuffer(4);var v1 = new Uint8Array(buffer);
v1[0] = 2;
v1[1] = 1;
v1[2] = 3;
v1[3] = 7;
...
var v1 = new Int32Array(b);var v2 = new Uint8Array(b, 2);var v3 = new Int16Array(b, 2, 2);
...
var a = new Uint8Array(8);var b = new Uint8Array(8);
...
var a = new Uint16Array(8);var b = new Uint16Array(10);
...
var a = new Uint16Array(8);var b = a.subarray(2,3);
...
var imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);var uint8ClampedArray = imageData.data;
...
来源网址： http://es6.ruanyifeng.com/#docs/set-map
作者： john-sky
Set和Map数据结构
...
var set = new Set(divs())set.size // 56
...
// 方法一
let set = new Set([1, 2, 3]);set = new Set([...set].map(val => val * 2));
// set的值是2, 4, 6

// 方法二
let set = new Set([1, 2, 3]);set = new Set(Array.from(set, val => val * 2));
// set的值是2, 4, 6
...
var data = {};var element = document.getElementById("myDiv");
...
let myMap = new Map().set(true, 7).set({foo: 3}, ['abc']);[...myMap]
// [ [ true, 7 ], [ { foo: 3 }, [ 'abc' ] ] ]
（2）数组转为Map
...
let myMap = new Map().set('yes', true).set('no', false);strMapToObj(myMap)
// { yes: true, no: false }
（4）对象转为Map
...
var wm = new WeakMap();var element = document.querySelector(".element");
...
来源网址： http://es6.ruanyifeng.com/#docs/iterator
作者： john-sky
Iterator和for...of循环
...
var $iterator = ITERABLE[Symbol.iterator]();var $result = $iterator.next();while (!$result.done) {
  var x = $result.value;
  // ...
...
var someString = "hi";typeof someString[Symbol.iterator]
// "function"
...
const arr = ['red', 'green', 'blue'];let iterator  = arr[Symbol.iterator]();
...
var engines = new Set(["Gecko", "Trident", "Webkit", "Webkit"]);for (var e of engines) {
  console.log(e);}
// Gecko
// Trident
// Webkit
...
let map = new Map().set('a', 1).set('b', 2);for (let pair of map) {
  console.log(pair);}
// ['a', 1]
// ['b', 2]
for (let [key, value] of map) {
...
let arr = ['a', 'b', 'c'];for (let pair of arr.entries()) {
  console.log(pair);}
// [0, 'a']
// [1, 'b']
// [2, 'c']
...
来源网址： http://es6.ruanyifeng.com/#docs/generator
作者： john-sky
Generator 函数
...
function* genFuncWithReturn() {
  yield 'a';
  yield 'b';
  return 'The result';}function* logReturned(genObj) {
...
function* F(){
  yield this.x = 2;
  yield this.y = 3;}var obj = {};var f = F.bind(obj)();
...
let bigArray = new Array(100000);for (let i = 0; i < 100000; i++) {
  bigArray[i] = i;}
...
function* loadUI() {
  ...
  hideLoadingScreen();}var loader = loadUI();
// 加载UI
loader.next()
// 卸载UI
loader.next()
...
来源网址： http://es6.ruanyifeng.com/#docs/promise
作者： john-sky
Promise对象
...
var p1 = new Promise(function (resolve, reject) {
  setTimeout(() => reject(new Error('fail')), 3000)})var p2 = new Promise(function (resolve, reject) {
...
getJSON("/post/1.json").then(function(post) {
  return getJSON(post.commentURL);}).then(function(comments) {
  // some code
}).catch(function(error) {
  // 处理前面三个Promise产生的错误
});
...
来源网址： http://es6.ruanyifeng.com/#docs/async
作者： john-sky
异步操作和Async函数
...
readFile(fileA).then(function(data){
  console.log(data.toString());}).then(function(){
  return readFile(fileB);}).then(function(data){
  console.log(data.toString());}).catch(function(err) {
  console.log(err);});
...
var ft = thunkify(f);var print = console.log.bind(console);ft(1, 2)(print);
// 3
...
var fs = require('fs');var thunkify = require('thunkify');var readFile = thunkify(fs.readFile);
...
来源网址： http://es6.ruanyifeng.com/#docs/class
作者： john-sky
Class
...
来源网址： http://es6.ruanyifeng.com/#docs/decorator#Trait
作者： john-sky
修饰器
...
来源网址： http://es6.ruanyifeng.com/#docs/module
作者： john-sky
Module
...
// 等同于
let _fs = require('fs');let stat = _fs.stat, exists = _fs.exists, readfile = _fs.readfile;
...
// lib.js
var counter = 3;function incCounter() {
  counter++;}
module.exports = {
  counter: counter,
...
console.log(foo);setTimeout(() => console.log(foo), 500);
...
$ babel-node
> import * as m from './even.js';> m.even(10);true> m.counter
6> m.even(20)true> m.counter
17
...
// even.js
var odd = require('./odd');var counter = 0;
...
来源网址： http://es6.ruanyifeng.com/#docs/style
作者： john-sky
编程风格
...
const inherits = require('inherits');function PeekableQueue(contents) {
  Queue.apply(this, contents);}inherits(PeekableQueue, Queue);
...
来源网址： http://es6.ruanyifeng.com/#docs/spec
作者： john-sky
读懂 ECMAScript 规格
...
If thisArg was supplied, let T be thisArg; else let T beundefined.
